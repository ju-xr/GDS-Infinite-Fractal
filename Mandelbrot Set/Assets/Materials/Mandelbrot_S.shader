...
Shader "Ju/Mandelbrot_S"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _Area;
            float _Angle, _MaxIter, _Color, _Repeat, _Speed, _Symmetry, _Animation, _Pattern;
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                if (_Pattern == 1) {
                    for (iter = 0; iter < _MaxIter; iter++)
                    {
                        zPrevious = rot(z, 0, _Time.y * _Animation);
                        //equation
                        z = float2(z.x * z.x - z.y * z.y, 2 * z.x * z.y) + c;
                ...
                //color
                float4 col = sin(float4(0.3, 0.45, 0.65, 1) * m * _Color) * 0.5 + 0.5; //Procedual colors
                //col = tex2D(_MainTex, float2(m, _Color));
                //col = tex2D(_MainTex, col* _Color);
                ...
                col *= 1+ sin(angle * _Animation + _Time.y*_Animation)* .2; // shadow + moving with speed
                return col;
            }
            ENDCG
        }
